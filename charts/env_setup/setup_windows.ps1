...
<#
.SYNOPSIS
    Charts Skill — Environment Setup for Windows (Win10/Win11)
.DESCRIPTION
    Detects platform, checks and installs all dependencies for the Charts skill.
...
$CHARTS_SKILL_DIR = Split-Path -Parent $ScriptDir
$env:CHARTS_SKILL_DIR = $CHARTS_SKILL_DIR

Write-Host "============================================"
Write-Host "  Charts Skill - Environment Setup"
Write-Host "  (Windows)"
Write-Host "============================================"
Write-Host ""
...
Write-Host "CHARTS_SKILL_DIR=$CHARTS_SKILL_DIR"
Write-Host ""
...
if ($UseChinaMirror) {
    ...
    # Auto-detect: try reaching pypi.org
    try {
...
if (-not $PyCmd) {
    ...
    Write-Info "Install option 3: choco install python3   (if Chocolatey installed)"
    if ($global:UseCN) {
        Write-Info "China alt: https://npmmirror.com/mirrors/python/ download installer"
    }
    $Errors++
}
Write-Host ""
...
Write-Host "--- [3/7] Python Packages (matplotlib, seaborn, numpy, adjustText) ---"
$PyPkgs = @(
    @{ Module = "matplotlib"; Package = "matplotlib" },
    @{ Module = "seaborn";    Package = "seaborn" },
    @{ Module = "numpy";      Package = "numpy" },
    @{ Module = "adjustText"; Package = "adjustText" }
)
...
try {
    ...
    Write-Info "Install option 2: https://nodejs.org/  download LTS installer"
    Write-Info "Install option 3: choco install nodejs-lts"
    if ($global:UseCN) {
        Write-Info "China alt: https://npmmirror.com/mirrors/node/ download installer"
    }
    $Errors++
}
...
Write-Host "--- [5/7] Playwright + Chromium ---"
try {
...
Write-Host "--- [6/7] Font Installation ---"
...
$CHARTS_FONTS_DIR = $UserFontDir
$env:CHARTS_FONTS_DIR = $CHARTS_FONTS_DIR
Write-Host ""

# ── Step 2g: CJK Font Verification ──
Write-Host "--- [7/7] CJK Font Verification ---"
$CjkFound = $false
