...
| Left margin | 3.17 cm | 1797 |
| Right margin | 3.17 cm | 1797 |
...
|---------|---------|---------|------|-------|
| H1 | Noto Sans SC | FreeSerif | 16 pt (size: 32) | Bold, centered |
| H2 | Noto Sans SC | FreeSerif | 15 pt (size: 30) | Bold |
| H3 | Noto Sans SC | FreeSerif | 14 pt (size: 28) | Bold |
| Body | Noto Serif SC | FreeSerif | 12 pt (size: 24) | |
| Caption | Noto Serif SC | FreeSerif | 10.5 pt (size: 21) | |

- Text color: always **pure black `"000000"`** (never dark-blue-grey)
- First-line indent: **480 twips** (2 chars at Noto Serif SC 12pt)
- Line spacing: **312** (1.3x).
- **Color routing for non-report documents**: When the document is a short-form text (essay, evaluation, letter, speech, application, reflection, etc.) rather than a structured report/whitepaper/proposal/consulting deliverable, heading color MUST use pure black `"000000"` instead of `palette.headingColor ?? palette.primary`. Colored headings are reserved for documents that need brand/professional identity (reports with covers, whitepapers, proposals, consulting deliverables).

### Profile B: Visual (resume, copywriting)

| Element | CN Font | EN Font | Size |
|---------|---------|---------|------|
| Name/Title | Noto Sans SC | FreeSerif | Varies |
| Body | Noto Sans SC | FreeSerif | 10–11 pt |
| Caption | Noto Sans SC | FreeSerif | 9 pt |

- First-line indent: **420 twips** (2 chars at Noto Sans SC)
...
| Element | Font | Size |
|---------|------|------|
| Red header org name | Noto Serif SC Bold | 26 pt (size: 52) |
| Title | Noto Serif SC Bold | 22 pt (size: 44) |
| Body | LXGW WenKai | 16 pt (size: 32) |
| Section heading | LXGW WenKai bold (or Noto Sans SC) | 16 pt (size: 32) |

- Line spacing: **560** (28 pt fixed)
- First-line indent: **640 twips** (2 chars at LXGW WenKai 16pt)
...
**Estimation for maxCharsPerLine:** For centered headings, estimate available width = page width - left margin - right margin. For Noto Sans SC at a given pt size, each CJK char ≈ pt × 20 twips wide. Divide available width by char width to get `maxCharsPerLine`.
...
| Standard fonts (Noto Sans SC, Noto Serif SC, LXGW WenKai, FreeSerif) | Available on both platforms |
...
- [ ] **Table colors derived from `palette.headerBg / palette.accentLine / etc.`** — no hardcoded hex for header bg/text/borders/zebra surface; never use raw `palette.accent` directly on body-page tables (see design-system.md § Table Color Token Derivation)
- [ ] **Heading colors derived from `coverPalettes[X].headingColor ?? coverPalettes[X].primary`** — no hardcoded `"0B1220"` or `"101820"`; `headingColor` is safe for white body pages even when `primary` is `"FFFFFF"`
...
- Use `P.titleColor` for title text
- Use `P.subtitleColor` for subtitle text
- Use `P.metaColor` for meta info text
- Use `P.footerColor` for footer text
...
```bash
python3 "$DOCX_SKILL_DIR/scripts/fix_footer_fields.py" output.docx
```

The script automatically: removes empty `<w:pgNumType/>` from cover section, patches footer instrText with correct format switches (`\* ROMAN` or `\* arabic`), and fixes `\* decimal` bugs.

See `toc.md` § Footer Field Instruction for full details.
