...
```bash
python3 "$DOCX_SKILL_DIR/scripts/add_toc_placeholders.py" output.docx --auto
```
...
# Manual entries
python3 "$DOCX_SKILL_DIR/scripts/add_toc_placeholders.py" output.docx \
...
WPS may ignore `pgNumType fmt` in the section properties. To ensure correct display, the footer PAGE field **MUST** include an explicit format switch.
...
- Detects each section's page number format (Roman vs Arabic) from `<w:pgNumType>`
- Patches bare `PAGE` → `PAGE \* arabic \* MERGEFORMAT` or `PAGE \* ROMAN \* MERGEFORMAT`
- Fixes `\* decimal` bug (causes "1decimal", "2decimal" rendering)
- Removes empty `<w:pgNumType/>` from cover sections
...
<details>
<summary>Manual JS post-processing (legacy — prefer the script above)</summary>
...
Also remove any empty `<w:pgNumType/>` from the cover section:
```js
