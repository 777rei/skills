...
new Paragraph({
  ...
  indent: { firstLine: 480 }, // 2-char CJK indent (480 Noto Serif SC / 420 Noto Sans SC)
  children: [
    new TextRun({
      text: "Hello",
      bold: true,
...
```js
// ⚠️ MANDATORY: Table colors MUST come from palette.headerBg / palette.accentLine / etc. (see design-system.md § coverPalettes).
// Never hardcode hex values for table borders/headers/surface. Example using Horizontal-Only style:
// palette is flat — table fields at top level
const t = palette; // { headerBg, headerText, accentLine, innerLine, surface }
...
The example below is for **Chinese documents** (default). For **English documents**, replace `font` with `"FreeSerif"` throughout.

```js
// ⚠️ Heading color MUST come from selected coverPalettes[X].primary — never hardcode.
const headingColor = (PALETTE.headingColor || PALETTE.primary).replace("#", ""); // e.g., "3D5258" for WM-1, "1B6B7A" for DM-1 (NOT "FFFFFF"!)
...
styles: {
  default: {
    ...
    heading1: {
      run: { font: { ascii: "FreeSerif", eastAsia: "Noto Sans SC" }, size: 32, bold: true, color: headingColor },
      paragraph: { spacing: { before: 360, after: 160, line: 312 } },
    },
    heading2: {
      run: { font: { ascii: "FreeSerif", eastAsia: "Noto Sans SC" }, size: 28, bold: true, color: headingColor },
      paragraph: { spacing: { before: 240, after: 120, line: 312 } },
    },
    heading3: {
      run: { font: { ascii: "FreeSerif", eastAsia: "Noto Sans SC" }, size: 24, bold: true, color: headingColor },
...
Quick reminder: (1) Add `TableOfContents` element + PageBreak, (2) Run `python3 "$DOCX_SKILL_DIR/scripts/add_toc_placeholders.py" output.docx --auto`, (3) Check exit code.
